...
func (lf *logFile) closeFile() {
	err := lf.file.Close()
	if err != nil {
		stderrSystemLogger.Errorf("close %s failed: %v", lf.filename, err)
	}
}
...
func (lf *logFile) reopenFile() {
	...
	err := lf.openFile()
	if err != nil {
		stderrSystemLogger.Errorf("open %s failed: %v", lf.filename, err)
		return
	}
}
...
func (lf *logFile) writeLog(p []byte) {
	...
	if lf.maxCacheCount == 0 {
		...
		if err != nil {
			stderrSystemLogger.Errorf("%v", err)
		}
		return
	}
	...
	if err != nil || len(p) != n {
		stderrSystemLogger.Errorf("write %s to cache failed: %v", p, err)
	}
	lf.cacheCount++
